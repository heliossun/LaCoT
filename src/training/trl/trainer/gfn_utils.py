...
def get_logits(model, rat_id, rat_lbl, batch, use_cache, dpo_forward, max_len, label_pad_token_id, pad_token_id,
    ...
    all_logits = model(
        input_ids,
        attention_mask=input_ids != label_pad_token_id,
        pixel_values=batch["pixel_values"].repeat(rat_id.shape[0], 1),
        image_grid_thw=batch["image_grid_thw"].repeat(rat_id.shape[0], 1),
        use_cache=use_cache,
    ).logits
    ...
    rational_logits = all_logits[:, skip_first:, :]
...
def get_generate_logits(model, batch, do_sample, temperature, max_len, min_len, cur_batch, min_step, reward_temperature,
   ...
   # generated = query_id
    ...
    #print(generated)
    #print(s)
    is_finished = torch.zeros(min_bs, dtype=torch.bool, device=query_id.device)
    with torch.no_grad():
        if use_cache:
            output = model(
                input_ids=generated[:, :-1],
                attention_mask=generated[:, :-1] != label_pad_token_id,
                pixel_values=batch["pixel_values"].repeat(min_bs, 1),
                image_grid_thw=batch["image_grid_thw"].repeat(min_bs, 1),
                use_cache=use_cache,
            )
            past_key_values = output.past_key_values
        ...
        for step in range(max_len - 2):
            ...
            # if is_finished.all():
            #print("is_finished: ",is_finished.shape)
